...
def create_nav_keyboard():
    ...
    keyboard.row(types.KeyboardButton("Проверка факта (/fact_check)"))
    keyboard.row(types.KeyboardButton("История запросов (/history)"))
    return keyboard
...
Выберите 1 из предложенных команд:
    """
    bot.reply_to(message, welcome_text, reply_markup=create_nav_keyboard())


# Обработчик текстовых сообщений (для кнопок)
@bot.message_handler(
    func=lambda message: message.text in ["Проверка факта", "История запросов"])
def handle_text_commands(message):
    """Обрабатывает текстовые команды от кнопок"""
    if message.text == "Проверка факта":
        handle_fact_check(message)
...
def handle_fact_check(message):
    ...
    msg = bot.reply_to(message, "Отправьте текст или факт для проверки:", reply_markup=types.ReplyKeyboardRemove())
...
def handle_history(message):
    ...
    if not history:
        bot.reply_to(message, "У вас пока нет истории запросов.", reply_markup=create_nav_keyboard())
        return
    ...
    bot.reply_to(message, response, reply_markup=create_nav_keyboard())
...
def process_fact_check(message):
    ...
    try:
        ...
        # Отправляем ответ с кнопками навигации
        bot.reply_to(message, fact_check_response, reply_markup=create_nav_keyboard())

    except Exception as e:
        bot.reply_to(message, f"Произошла ошибка: {str(e)}", reply_markup=create_nav_keyboard())
