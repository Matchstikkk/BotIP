...
# URL API Facticity
FACTICITY_API_URL = "https://app.facticity.ai/c/b48e0da6-bca7-4969-817a-02ff0e3985ff"
...
def get_fact_check(query: str) -> str:
    ...
    try:
        ...
        headers = {
            ...
            "Authorization": f"Bearer {FACTICITY_API_KEY}"  # Используем API-ключ
        }
        # Тело запроса
        data = {"query": query}  # Предполагаем, что API принимает поле 'query'

        # Отправляем POST-запрос к API Facticity
        response = requests.post(FACTICITY_API_URL, json=data, headers=headers)
